...
st.write("Upload photos of highlighted book pages to extract vocabulary directly into your **Capybara** Anki CSV format using **Claude Sonnet 5**.")
...
api_key_input = st.sidebar.text_input("Claude API Key", value=secret_key, type="password", help="Enter your Anthropic API key")
...
if uploaded_files:
    if not api_key:
        st.error("Please enter a Claude API Key in the sidebar to proceed.")
    else:
        if st.button("Process Images"):
            st.session_state.cards = []
            progress_bar = st.progress(0)
            
            with st.spinner("Processing images with Claude Sonnet 5..."):
